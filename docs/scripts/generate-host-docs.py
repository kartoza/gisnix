...
# Two roots, only ever the same directory when this runs inside gisnix's
# own checkout (true for every run until a downstream flake started
# consuming gisnix — see utils/lib/hostconfig.py for the same split done
# for `gisnix configure`). GISNIX_ROOT is where the bundle catalogue's
# published metadata lives (docs/references/software.json — gisnix's own
# docs-generate-software already wrote it, a downstream consumer doesn't
# regenerate it, just reads gisnix's committed copy). TARGET_ROOT is the
# flake actually being documented: its own hosts, its own docs/hosts/ to
# write into, its own flake.nix/software/profiles/etc. to cache-key
# against — always the caller's own repo root.
...
HOSTS_DIR = TARGET_ROOT / "docs" / "hosts"
# Passed explicitly to diagram.render() (its own default is gisnix's own
# docs/assets/diagrams, correct only for macros.py's hand-written pages)
# — per-host/fleet diagrams belong beside this consumer's own docs/hosts/.
...
def nix_eval(attr: str, apply: str | None = None, optional: bool = False) -> Any:
    ...
    try:
        out = subprocess.run(
            cmd, cwd=TARGET_ROOT, check=True, capture_output=True, text=True,
        )
...
def load_catalogue() -> dict[str, dict[str, Any]]:
    ...
    GISNIX_ROOT/docs/references/software.json — gisnix's own published
    catalogue, generated by generate-software-catalogue.py and committed
    to gisnix's repo (regenerateDocs, in gisnix's own flake.nix, runs that
    generator BEFORE this script when building gisnix's own docs site —
    for a downstream consumer this file is just gisnix's already-published
    copy, not something to regenerate against the consumer's own
    packages). Missing or unreadable = empty: the host pages then render
    with "—" descriptions rather than failing the docs build."""
    try:
        return json.loads((GISNIX_ROOT / "docs" / "references" / "software.json").read_text())
...
CACHE_DIR = TARGET_ROOT / ".cache" / "docs-host-facts"
...
def _cache_key(host: str) -> str:
    ...
    for path in sorted(paths):
        digest.update(str(path.relative_to(TARGET_ROOT)).encode())
        try:
...
def main() -> int:
    ...
    for page in sorted(HOSTS_DIR.glob("*.md")):
        ...
        if page.stem in keep or owner in keep:
            continue
        print(f"  removing stale page: {page.relative_to(TARGET_ROOT)}", flush=True)
        page.unlink()

    print(
        f"Wrote {len(hosts)} host pages + index.md to "
        f"{HOSTS_DIR.relative_to(TARGET_ROOT)}/"
    )
    return 0
