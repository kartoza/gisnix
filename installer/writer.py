...
def render_disks_nix(state: InstallState) -> str:
    if state.storage_mode == STORAGE_ZFS_ENCRYPTED_SINGLE:
        device = state.disks[0]
        plan = sizing.quota_plan(
            state.disk_sizes[device],
        ...
        return (
            ...
            "# Dataset quotas sized against this disk's real capacity by installer/sizing.py,\n"
            f"# leaving a {sizing.DEFAULT_HEADROOM_BYTES // sizing.GIB}G headroom reserve unclaimed by any quota.\n"
            '{ gisnixRoot, ... }:\n'
            f'import (gisnixRoot + "/templates/disko/zfs-encrypted-single.nix") {{\n'
            f'  device = "{device}";\n'
            f'  rootQuota = "{plan["root"]}";\n'
            f'  nixQuota = "{plan["nix"]}";\n'
            f'  homeQuota = "{plan["home"]}";\n'
            f'  overflowQuota = "{plan["overflow"]}";\n'
            "}\n"
        )
    if state.storage_mode == STORAGE_XFS_SINGLE:
        return (
    ...
    if state.storage_mode == STORAGE_ZFS_MULTI:
        ...
        usable = sizing.multi_disk_usable_bytes(
            [state.disk_sizes[d] for d in state.disks],
            esp_bytes=ESP_SIZE_BYTES,
            mode=state.zfs_raid_mode,
        )
        plan = sizing.quota_plan(
            usable,
            esp_bytes=0,
            datasets=("root", "nix", "home"),
        )
        return (
            f"# ZFS, {len(state.disks)} disks, {state.zfs_raid_mode}. Generated by the gisnix installer.\n"
            "# Dataset quotas sized against the pool's estimated usable capacity by\n"
            f"# installer/sizing.py, leaving a {sizing.DEFAULT_HEADROOM_BYTES // sizing.GIB}G headroom reserve unclaimed.\n"
            '{ gisnixRoot, ... }:\n'
