...
# `kz update foobar` can build it. The generated host starts minimal — a
...
#   2. nix-shell -p git
#   3. clone this flake
#   4. kz create-host foobar          <- you are here
#   5. kz update foobar
#   6. reboot into the new system
#   7. uncomment bundles as you need them, kz update again
...
usage() {
  cat <<EOF
Usage: kz create-host <name> [--dry-run]
...
gen_hardware() {
  ...
  cat <<EOF
# Hardware for ${NAME} — GENERATED by \`kz create-host\` on $(date -u +%Y-%m-%d).
#
...
#           ${GPU_MODEL}
#
# Re-run \`kz generate-hardware\` after a hardware change.
...
gen_disks() {
  cat <<EOF
# Filesystems for ${NAME} — GENERATED by \`kz create-host\` on $(date -u +%Y-%m-%d).
#
...
gen_networking() {
  cat <<EOF
# Networking for ${NAME} — GENERATED by \`kz create-host\`.
...
# ${NAME} — ${DESCRIPTION}
#
# Created by \`kz create-host\` from a self-installed NixOS system.
#
...
# unstaged hosts/<name>/ is invisible to `kz update` and the error it
...
if nix eval --raw ".#nixosConfigurations.${NAME}.config.system.build.toplevel.drvPath" >/dev/null 2>&1; then
  ...
  warn "Fix that before running kz update."
fi
...
cat <<EOF
  ...
  3. Reboot into it, then add software a bundle at a time
       Uncomment ONE line in hosts/$NAME/config.nix, run kz update $NAME,
