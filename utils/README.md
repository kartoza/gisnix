...
| `kz` | `kz` alone prints the cheat-sheet; `kz <command>` runs one |
| `<leader>p<key>` | `.nvim.lua` reads the same manifest at startup |

So the current list is always:

```bash
kz                    # the cheat-sheet, from inside `nix develop`
```

A row whose script does not exist yet is skipped by the flake and shown
dimmed by `kz`. The manifest describes the intended lifecycle; the marker
...
| `dev-help.sh` | renders the cheat-sheet; invoked by `kz` and on shell entry |
| `nvidia-launch.sh` | launch an application on the NVIDIA GPU |
| `test-install.sh` | exercise a host installation in a throwaway VM |
