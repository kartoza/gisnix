# Getting back into a machine when something in the initrd goes wrong,
# remotely or at the console. On by default for every host.
#
# Remote: a minimal SSH server in the initrd, so a machine whose encrypted
# ZFS root is waiting on a passphrase (or that's just stuck partway through
# boot) can be reached and rebooted from elsewhere. `utils/unlock-host.sh`
# (the "unlock-host" package in utilities.nix) is the client half of this,
# already ships everywhere.
#
...
#
# Local: `boot.initrd.systemd.emergencyAccess` defaults to `false` in
# upstream NixOS, which doesn't mean "no password" — sulogin treats it as a
